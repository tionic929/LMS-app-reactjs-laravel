<!DOCTYPE html>
<html>
<head>
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
    <title>Verification Code</title>
</head>
<body style="font-family: Arial, sans-serif; background-color: #f4f4f4; margin: 0; padding: 0;">

    <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="background-color: #f4f4f4; height: 100%;">
        <tr>
            <td align="center" style="padding: 25px 0;">
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 8px; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1);">
                    
                    <tr>
                        <td align="center" style="padding: 20px 20px 10px; border-bottom: 1px solid #e5e5e5;">
                            <h1 style="color: #4f46e5; font-size: 24px; margin: 0;">LMS Team</h1>
                        </td>
                    </tr>

                    <tr>
                        <td style="padding: 30px 40px; text-align: center;">
                            <h2 style="color: #1f2937; font-size: 20px; font-weight: bold; margin-bottom: 20px;">Your Verification Code</h2>

                            <p style="color: #374151; font-size: 16px; line-height: 1.6; margin-bottom: 25px;">
                                Please use the following code to complete your verification process:
                            </p>

                            <table width="100%" cellspacing="0" cellpadding="0" style="margin-bottom: 30px;">
                                <tr>
                                    <td align="center">
                                        <div style="
                                            display: inline-block;
                                            padding: 15px 30px;
                                            background-color: #f0f4ff; /* Light blue background for emphasis */
                                            color: #4f46e5; /* Primary color for code */
                                            font-size: 32px; /* Large font size */
                                            font-weight: bold;
                                            letter-spacing: 5px; /* Spacing out the digits */
                                            border: 2px dashed #4f46e5; /* Dashed border for attention */
                                            border-radius: 8px;
                                            user-select: all; /* Allows easy copying on some clients */
                                            -webkit-user-select: all;
                                            -moz-user-select: all;
                                        ">
                                            {{ $token }}
                                        </div>
                                    </td>
                                </tr>
                            </table>

                            <p style="color: #6b7280; font-size: 14px; line-height: 1.6; margin-bottom: 15px;">
                                This code is valid for a limited time. Please return to the application to enter it now.
                            </p>
                            
                            <p style="color: #6b7280; font-size: 14px; line-height: 1.6; border-top: 1px solid #e5e5e5; padding-top: 20px; margin-top: 30px;">
                                If you did not request this code, you can safely ignore this email.
                            </p>
                        </td>
                    </tr>

                    <tr>
                        <td align="center" style="padding: 20px 40px; background-color: #f9fafb; border-top: 1px solid #e5e5e5;">
                            <p style="color: #9ca3af; font-size: 12px; margin: 0;">
                                Thanks,<br>LMS Team
                            </p>
                        </td>
                    </tr>

                </table>
            </td>
        </tr>
    </table>
</body>
</html>