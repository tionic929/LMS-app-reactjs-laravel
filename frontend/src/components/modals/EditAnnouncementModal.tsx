...
interface EditAnnouncementModalProps {
  ...
  onClose: () => void;
  onSubmit: (id: number, payload: { title: string; content: string; type: string }) => Promise<void>;
}
...
const EditAnnouncementModal: React.FC<EditAnnouncementModalProps> = ({ show, announcement, onClose, onSubmit }) => {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await onSubmit(announcement.id, { title: title.trim(), content: content.trim(), type });
      onClose();
    } catch (err: any) {
